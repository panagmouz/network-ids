...
class NslKdd:
    def __init__(self, testCase, isMulticlass=False):
        ...
        train = '.dataset_files/NSL-KDD/KDDTrain+.txt'
        test = '.dataset_files/NSL-KDD/KDDTest+.txt'
        test21 = '.dataset_files/NSL-KDD/KDDTest-21.txt'
        ...
        self.binary_flags = [
            'normal', 'ipsweep', 'nmap', 'portsweep', 'satan', 'saint', 'mscan', 'back', 'land', 'neptune', 'pod',
            'smurf',
            'teardrop', 'apache2', 'udpstorm', 'processtable', 'mailbomb', 'buffer_overflow', 'loadmodule', 'perl',
            'rootkit',
        ...
        self.test_data_21 = pd.read_csv(test21, names=feature_list)
        self.train_data['mode'] = 'Train'
        self.test_data['mode'] = 'Test'
        self.test_data_21['mode'] = 'Test'
    ...
    def get(self):
        if self.testCase == 'test-21':
            data = pd.concat([self.train_data, self.test_data_21])
        elif self.testCase == 'test':
        ...
        data = data.drop(columns='difficulty', axis=1)
        return data


if __name__ == "__main__":
    dataset = NslKdd(testCase="test-21", isMulticlass=True)
    dataset.graphs()
    print(dataset.get())
